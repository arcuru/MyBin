...
QDisplay::QDisplay(QApplication& a) :
	ready(true), closed(false), paused(false), qTimer(nil), image(nil), app(a),
...
// Member functions
bool QDisplay::Load(const char* fn) { // Load an image from a file
	image = new QImage();
	if (!image->load(fn)) { // load failed
		delete image;
		image = nil;
		return false;
	}
...
void QDisplay::BlankImage(int w, int h, int d) { // Create a blank (all white) image
	int colorCount;
	...
	if (d == 8) {
		...
		if (whitew != w || whiteh != h || whited != d) { // Existing white buffer wrong size
			delete[] white;
			white = new unsigned char[w * h];
			memset(white, 0xff, w * h);
			whitew = w;
	...
	if (image) { // Delete any existing
		delete image;
		image = nil;
	}

	// Create an image, all white
	image = new QImage(white, w, h, d, colorTable, colorCount,
			QImage::IgnoreEndian);
	resize(image->width(), image->height()); // Size the main window to image
...
int QDisplay::Depth() { // Return image depth (bits per pixel), either 1, 8, or 32.
...
int QDisplay::Width() { // Return image width (pixels)
	if (!image)
		return 0;
	return image->width();
}

int QDisplay::Height() { // Return image height (pixels)
	if (!image)
		return 0;
	return image->height();
}

unsigned char* QDisplay::ImageData() { // Get a pointer to the image data
	if (!image)
		return nil; // No image loaded
...
void QDisplay::Update() {
	if (closed)
		return; // No more updates after window closed
	while (paused)
		app.processEvents(); // Wait while paused
	while (!ready && !closed)
		app.processEvents(); // Wait for 25fps elapsed
	if (updateRate) { // Non-zero update rate specified
		ready = false;
#ifdef QT4
