...
unsigned char* SieveEratosthenesThreaded(int N) {
	...
	pthread_t thread[PRIMES_CC_NTHREADS];
	pthread_attr_t attr;
	pthread_attr_init(&attr);
	pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_JOINABLE);
	SieveEratosthenesStruct inputs[PRIMES_CC_NTHREADS];
	...
	int t, rc;
	int currentPos = 0;
	for (t = 0; t < PRIMES_CC_NTHREADS; t++) {
...
static void *primeListThread(void* input) {
	...
	int begin = (primeEnd / PRIMES_CC_NTHREADS) * threadid;
	int end = 0;
	if (threadid + 1 != PRIMES_CC_NTHREADS)
		end = (primeEnd / PRIMES_CC_NTHREADS) * (threadid + 1);
	else
		end = primeEnd + 1;
...
int* primeListEThreaded(int N) {
	return primeListEThreaded(N, SieveEratosthenes(N));
}

int* primeListEThreaded(int N, unsigned char* prime) {
	int primeEnd = N / 30;
	int n, k;
	unsigned char s;
	int top = 3;
	int spread = primeEnd / PRIMES_CC_NTHREADS;
	int spreadnum = 0;
	int* StartInd = (int*) malloc(sizeof(int) * PRIMES_CC_NTHREADS);
	for (n = 0; n <= primeEnd; n++) { //Counts primes
		if (n == spreadnum * spread && spreadnum < PRIMES_CC_NTHREADS) {
	...
	pthread_attr_init(&attr);
	pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_JOINABLE);
	primeListStruct inputs[PRIMES_CC_NTHREADS];
	int t, rc;
	for (t = 0; t < PRIMES_CC_NTHREADS; t++) {
		inputs[t].primes = prime;
		inputs[t].list = list;
...
static void *primeIndexThread(void* input) {
	...
	int n, k;
	unsigned char s;
	for (n = threadid; n <= primeEnd; n += PRIMES_CC_NTHREADS) {
		for (k = 0, s = 1; s; k++, s += s) {
			if (!(prime[n] & s)) {
				switch (k) {
				case 0:
...
unsigned char* primeIndexListEThreaded(int N) {
	return primeIndexListEThreaded(N, SieveEratosthenes(N));
}

unsigned char* primeIndexListEThreaded(int N, unsigned char* prime) {
	...
	pthread_attr_init(&attr);
	pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_JOINABLE);
	primeIndexStruct inputs[PRIMES_CC_NTHREADS];
	int t, rc;
	for (t = 0; t < PRIMES_CC_NTHREADS; t++) {
		inputs[t].primes = prime;
		inputs[t].list = list;
