...
unsigned char* SieveEratosthenesThreaded(int N) {
	...
	pthread_t thread[PRIMES_C_NTHREADS];
	pthread_attr_t attr;
	pthread_attr_init(&attr);
	pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_JOINABLE);
	SieveEratosthenesStruct inputs[PRIMES_C_NTHREADS];
...
static void *primeListThread(void* input) {
	...
	int begin = (primeEnd / PRIMES_C_NTHREADS) * threadid;
	int end = 0;
	if (threadid + 1 != PRIMES_C_NTHREADS)
		end = (primeEnd / PRIMES_C_NTHREADS) * (threadid + 1);
	else
		end = primeEnd + 1;
...
int* primeListEThreaded_inc(int N, unsigned char* prime) {
	...
	unsigned char s;
	int top = 3;
	int spread = primeEnd / PRIMES_C_NTHREADS;
	int spreadnum = 0;
	int* StartInd = (int*) malloc(sizeof(int) * PRIMES_C_NTHREADS);
	for (n = 0; n <= primeEnd; n++) { //Counts primes
		if (n == spreadnum * spread && spreadnum < PRIMES_C_NTHREADS) {
	...
	pthread_attr_init(&attr);
	pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_JOINABLE);
	primeListStruct inputs[PRIMES_C_NTHREADS];
	int t, rc;
	for (t = 0; t < PRIMES_C_NTHREADS; t++) {
		inputs[t].primes = prime;
		inputs[t].list = list;
...
static void *primeIndexThread(void* input) {
	...
	int n, k;
	unsigned char s;
	for (n = threadid; n <= primeEnd; n += PRIMES_C_NTHREADS) {
		for (k = 0, s = 1; s; k++, s += s) {
			if (!(prime[n] & s)) {
				switch (k) {
				case 0:
...
unsigned char* primeIndexListEThreaded_inc(int N, unsigned char* prime) {
	...
	pthread_attr_init(&attr);
	pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_JOINABLE);
	primeIndexStruct inputs[PRIMES_C_NTHREADS];
	int t, rc;
	for (t = 0; t < PRIMES_C_NTHREADS; t++) {
		inputs[t].primes = prime;
		inputs[t].list = list;
